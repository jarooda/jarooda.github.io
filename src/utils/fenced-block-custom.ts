...
class AstroFencedBlock extends HTMLElement {
  constructor() {
    super()

    const preBlocks: HTMLPreElement[] = Array.from(this.querySelectorAll("pre"))
    ...
    function renderCodeLang(preBlock: HTMLPreElement): void {
      ...
      if (lang !== "plaintext") {
        ...
        langSection.innerHTML = languages[lang] || lang
...
// Tell the browser to use our AstroFencedBlock class for <astro-fenced-block> elements.
customElements.define("astro-fenced-block", AstroFencedBlock)
